<?php //Order Edit
// Heading
$_['heading_title']		= 'Order Create:';
$_['heading_description']	= 'You can create orders here.';

// Text
$_['text_message']		= 'Success: You have created a new order!';

// Text
$_['text_order']		= 'Order Reference';
$_['text_email']		= 'E-Mail';
$_['text_telephone']		= 'Telephone';
$_['text_fax']			= 'Fax';
$_['text_shipping_address']	= 'Shipping Address';
$_['text_shipping_method']	= 'Shipping Method';
$_['text_payment_address']	= 'Payment Address';
$_['text_payment_method']	= 'Payment Method';
$_['text_order_history']	= 'Order History';
$_['text_order_download']	= 'Order Downloads';
$_['text_order_update']		= 'Order Update';
$_['text_product']		= 'Product';
$_['text_model_number']		= 'Model Number';
$_['text_soldby']		= 'Dispatched from:';
$_['text_quantity']		= 'Quantity';
$_['text_price']		= 'Regular Price';
$_['text_total']		= 'Total';
$_['text_error']		= 'The invoice you requested could not be found!';
$_['text_currency']		= 'Currency : ';
$_['text_tax']			= 'Tax Included in Amount';
$_['text_tax_percent']		= 'Tax %';
$_['text_special']		= 'Special<br>Price';
$_['text_coupon_value']		= 'Coupon<br>Value';
$_['text_discount_value']	= 'Discount<br>Amount';
$_['text_tax_rate']		= 'Tax<br> Rate';
$_['text_extended']		= 'Extended<br>Amount';
$_['text_net']			= 'Net Cost';
$_['text_cart_totals']		= 'Cart Totals';
$_['text_product_totals']	= 'Product Totals';
$_['text_tax_amount']		= 'Tax';
$_['text_shipping_cost']	= 'Shipping Cost';
$_['text_free_shipping']	= 'Free Shipping';
$_['text_shipping']		= 'Ship';
$_['text_shippable']		= 'Shippable';
$_['text_non_shippable']	= 'Non Shippable';
$_['text_invoice_number']	= 'Invoice Number';
$_['text_downloadable']		= 'Downloadable';
$_['text_select_shipping']	= 'Select Shipping';
$_['text_select_payment']	= 'Select Payment';
$_['text_coupon_total']     = 'Coupon total';
$_['text_comment_cancelled']= 'Invoice Cancelled'; 
$_['text_discount_total']   = 'Discount:';

// Column
$_['column_order_id']		= 'Order ID';
$_['column_reference']		= 'Reference';
$_['column_invoice_number']	= 'Invoice Number';
$_['column_name']		= 'Customer Name';
$_['column_status']		= 'Status';
$_['column_date_added']		= 'Date Added';
$_['column_total']		= 'Total';
$_['column_notify']		= 'Customer Notified';
$_['column_comment']		= 'Comments';
$_['column_download']		= 'Download Name';
$_['column_filename']		= 'Filename';
$_['column_remaining']		= 'Remaining';
$_['column_action']		= 'Action';

// Tabs
$_['tab_original']		= 'Original<br>Order';
$_['tab_create']		= 'Create New<br>Order';

// Entry 
$_['entry_status']			= 'Status:';
$_['entry_comment']			= 'Comments:';
$_['entry_notify']			= 'Notify Customer:';
$_['entry_customer']		= 'Select Customer';
$_['entry_address']			= 'Select Shipping Address';
$_['entry_select_product']	= 'Select Product';
$_['entry_shipping']		= 'Shipping:';
$_['entry_subtotal']		= 'Subtotal:';
$_['entry_totals_subtotal'] = 'Subtotal:';
$_['entry_totals_coupon']   = 'Coupon Discount:';
$_['entry_totals_discount'] = 'Discount:';
$_['entry_totals_shipping'] = 'Shipping:';
$_['entry_totals_freeshipping']= 'Coupon(Free Shipping):';
$_['entry_totals_tax']      = 'Tax:';
$_['entry_total_invoice']	= 'Total:';
$_['entry_freeshipping_net'] = 'Coupon(Free Shipping):';
$_['entry_coupon_total']     = 'Coupon Discount:';
$_['entry_discount_total'] = 'Discount:';
$_['entry_total_tax']		= 'Tax:';
$_['entry_shipping_tax']	= 'Shipping Tax Rate %: ';
$_['button_add']			= 'Add Product';
$_['button_submit']			= 'Submit Entry';
$_['button_create']			= 'Submit Order';

// E-Mail 
$_['email_subject']		= 'Order ID #%s';
$_['email_greeting']		= 'Dear %s,';
$_['email_thanks']		= 'Thank you for your interest in %s Products. Your order (whose details are listed below) has been received.';
$_['email_order']		= '<b>Order ID:</b> #%s';
$_['email_invoice_number']	= '<b>Invoice Number:</b> #XXXZZZ#';
$_['email_invoice']		= '<b>Invoice:</b> <a href="%s">%s</a>';
$_['email_date']		= '<b>Date Ordered:</b> %s';
$_['email_email']		= 'E-Mail';
$_['email_telephone']		= 'Telephone';
$_['email_fax']			= 'Fax';
$_['email_shipping_address']	= 'Shipping Address';
$_['email_shipping_method']	= 'Shipping Method';
$_['email_payment_address']	= 'Payment Address';
$_['email_payment_method']	= 'Payment Method';
$_['email_comment']		= 'Your Order Comments';
$_['email_thanks_again']	= 'Thanks again for choosing %s.';  
$_['email_product']		= 'Product';  
$_['email_model_number']	= 'Model Number';  
$_['email_soldby']		= 'Dispatched from:';
$_['email_quantity']		= 'Quantity';  
$_['email_price']		= 'Unit Price';  
$_['email_total']		= 'Total';
$_['email_ship']		= 'Yes';
$_['email_noship']		= 'Pickup';
$_['email_download']		= 'Download';
$_['email_message']		= 'Thank you for your interest in %s Products. Your order has been received and will be dealt with as quickly as possible.' . "\n\n" . 
				'Order ID: #%s' . "\n" .
				'Invoice Number: #XXXZZZ#' . "\n" .
				'Invoice: %s ' . "\n" .
				'Date Ordered: %s' . "\n\n" .
				'The comments for your order are:' . "\n\n" . '%s' . "\n\n" .
				'Please reply to this email if you have any questions.' . "\n\n";

// Error
$_['error_permission']		= 'Warning: You do not have permission to create orders';
?>
