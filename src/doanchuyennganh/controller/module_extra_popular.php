...
class ControllerModuleExtraPopular extends Controller {
	...
	function index() { 
		...
		if ($this->request->has('catalog_popular_ratings', 'post')) {
			$view->set('catalog_popular_ratings', $this->request->gethtml('catalog_popular_ratings', 'post'));
		} else {
			$view->set('catalog_popular_ratings', @$setting_info['catalog']['popular_ratings']);
		}
