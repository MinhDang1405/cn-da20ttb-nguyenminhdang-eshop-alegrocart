<?php //Search AdminModule AlegroCart
class ControllerModuleCatalogsearch extends Controller {
	...
	function index() { 
		...
		if ($this->request->isPost() && $this->request->has('catalog_search_status', 'post') && $this->validate()) {
		...
		if ($this->request->has('catalog_search_status', 'post')) {
			$view->set('catalog_search_status', $this->request->gethtml('catalog_search_status', 'post'));
		} else {
	...
	function install() {
		if ($this->user->hasPermission('modify', 'module_catalog_search')) {
			$this->modelSearch->delete_search();
			$this->modelSearch->install_search();
			$this->session->set('message', $this->language->get('text_message'));
		} else {
