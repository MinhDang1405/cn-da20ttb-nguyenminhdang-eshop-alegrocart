...
class ModuleFeatured extends Controller {
	...
	function fetch() {
		...
		if ($config->get('featured_status')) {
			...
			$featured_results = $this->modelProducts->get_featured();

			$maxrow = count($featured_results)-1;
			if ($featured_results) {
				if ($maxrow < $limit){
					$results = $featured_results;
				} else {
					$i = 0;
					while ($i < $limit){
						$rand->uRand(0,$maxrow);
					...
					foreach ($rand->RandomNumbers as $mykey){
						$product_rand[$i] = $featured_results[$mykey];
						$i ++;
					}
		...
		foreach ($results as $result) {
				if ($config->get('featured_ratings')) {
