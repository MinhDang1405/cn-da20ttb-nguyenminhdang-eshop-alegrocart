...
class ModuleSpecials extends Controller {
	...
	if ($config->get('specials_status')) {
		...
		$view->set('heading_title', $language->get('heading_title'));
			...
			if ($config->get('specials_slider')){
				$limit = $config->get('specials_slimit') == '0' ? '': (int)$config->get('specials_slimit');
				$columns = $config->get('specials_scolumns');
			} else {
				$limit = $config->get('specials_limit') == '0' ? '': (int)$config->get('specials_limit');
				$columns = $config->get('specials_columns');
			}
			...
			if($columns == 1 && $location == 'content'){
				$image_width = $config->get('specials_image_width');
				$image_height = $config->get('specials_image_height');
			} else if ($columns <= 3){
				$image_width = $config->get('specials_image_width') <= 175 ? $config->get('specials_image_width') : 175;
				$image_height = $config->get('specials_image_height') <= 175 ? $config->get('specials_image_height') : 175;
			} else {
			...
			$specials_results = $this->modelProducts->get_specials();

			$maxrow = count($specials_results)-1;
			if ($specials_results) {
				if ($maxrow < $limit){
					...
					foreach ($rand->RandomNumbers as $mykey){
						$product_rand[$i] = $specials_results[$mykey];
						$i ++;
					}
		...
		foreach ($results as $result) {
				if ($config->get('specials_ratings')) {
				...
				if ($location == 'content' && $columns == 1) {
					$desc = formatedstring($result['description'],$config->get('specials_lines_single'));
					// Product Discounts
					...
					if ($result['alt_description']){
						$desc = strippedstring($result['alt_description'],$config->get('specials_lines_char'));
					} else {
					...
					if ($result['alt_description']){
						$desc = formatedstring($result['alt_description'],$config->get('specials_lines_multi'));
					} else {
			...
			$view->set('slider', $config->get('specials_slider'));
			...
			$view->set('addtocart',$config->get('specials_addtocart'));
