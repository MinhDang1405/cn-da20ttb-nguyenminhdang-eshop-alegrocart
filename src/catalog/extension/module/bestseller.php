	...
	if ($config->get('bestseller_status')) {
		...
		$view = $this->locator->create('template');
			...
			if($columns == 1 && $location == 'content'){
				...
				$image_width = $config->get('bestseller_image_width') <= 175 ? $config->get('bestseller_image_width') : 175;
				$image_height = $config->get('bestseller_image_height') <= 175 ? $config->get('bestseller_image_height') : 175;
			} else {
				$image_width = $config->get('bestseller_image_width') <= 140 ? $config->get('bestseller_image_width') : 140;
				$image_height = $config->get('bestseller_image_height') <= 140 ? $config->get('bestseller_image_height') : 140;
			}
			...
			foreach ($results as $result) {
				if ($config->get('bestseller_ratings')) {
				...
				if ($location == 'content' && $columns == 1) {
					$desc = formatedstring($result['description'],$config->get('bestseller_lines_single'));
					// Product Discounts
					...
					if ($result['alt_description']){
						...
						$desc = strippedstring($result['description'],$config->get('bestseller_lines_char'));
					}
					$product_discounts = '';
					...
					if ($result['alt_description']){
						...
						$desc = formatedstring($result['description'],$config->get('bestseller_lines_multi'));
					}
					$product_discounts = '';
					$options = $this->modelProducts->get_option_names($result['product_id']);
					$product_options = FALSE;
			...
			$view->set('image_display', $config->get('bestseller_image_display'));
