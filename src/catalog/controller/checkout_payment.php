...
class ControllerCheckoutPayment extends Controller {
	...
	function index() {
		...
		if (count($this->payment->getMethods())==1) {
			$this->session->set('payment_method', key($this->payment->getMethods()));
			$this->session->set('one_payment_method', 1);
			$this->response->redirect($this->url->ssl('checkout_confirm'));
		} else {
			$this->session->delete('one_payment_method');
		}
	...
	function comment(){
		if ($this->request->isPost()) {
			$this->session->set('comment', $this->request->sanitize('Comment', 'post'));
			$this->session->set('payment_method', $this->request->gethtml('paymentMethod', 'post'));
			...
			elseif ($this->request->gethtml('Button', 'post') == "change_address") {
				$response=array("status"=>'P_ADDRESS_SUCCESS');
			}
